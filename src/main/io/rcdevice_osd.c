...
int rcdeviceOSDWriteString(displayPort_t *displayPort, uint8_t x, uint8_t y, const char *text, textAttributes_t attr);
{
    ...
    runcamDeviceDispWriteHorizontalString(osdDevice, x, y, text);
#else
    for (int xpos = x; *text && xpos < columnCount; xpos++) {
        screenBuffer[y * columnCount + xpos] = *text++;
    }
#endif
    return 0;
}

int rcdeviceOSDWriteChar(displayPort_t *displayPort, uint8_t x, uint8_t y, uint8_t c, textAttributes_t attr)
{
    UNUSED(displayPort);
    UNUSED(attr);
#if !defined(USE_PARTICLE_DRAW)
