...
typedef struct testData_s {
    ...
    uint8_t responesBufs[MAX_RESPONSES_COUNT][60];
...
TEST(RCDeviceTest, TestInitDevice)
{
    ...
    uint8_t responseData[] = { 0xCC, 0x01, 0x37, 0x90 };
...
TEST(RCDeviceTest, TestInitDeviceWithInvalidResponse)
{
    ...
    uint8_t responseData[] = { 0xCC, 0x01, 0x37, 0x90, 0x33 };
    addResponseData(responseData, sizeof(responseData), true);
    bool result = runcamDeviceInit(&device);
    EXPECT_EQ(result, true);
    clearResponseBuff();
    ...
    // invalid crc
    uint8_t responseDataWithInvalidCRC[] = { 0xCC, 0x01, 0x37, 0x91 };
...
TEST(RCSplitTest, Test5KeyOSDCableSimulationProtocol)
{
    memset(&testData, 0, sizeof(testData));
    testData.isRunCamSplitOpenPortSupported = true;
    testData.isRunCamSplitPortConfigurated = true;
...
TEST(RCSplitTest, Test5KeyOSDCableSimulationWithout5KeyFeatureSupport)
{
...
TEST(RCSplitTest, Test5KeyOSDCableSimulationWith5KeyFeatureSupport)
{
    // test simulation without device init
    rcData[THROTTLE] = 1500; // THROTTLE Mid
    rcData[ROLL] = 1500; // ROLL Mid
    ...
    testData.isAllowBufferReadWrite = true;
    testData.maxTimesOfRespDataAvailable = 0;
    uint8_t responseData[] = { 0xCC, 0x01, 0x3F, 0xb9 };
    addResponseData(responseData, sizeof(responseData), true);
    bool result = rcdeviceInit();
    EXPECT_EQ(result, true);
    clearResponseBuff();
...
TEST(RCSplitTest, TestOSDDeviceSupportWithAutoDetectVideoSystem)
{
    memset(&testData, 0, sizeof(testData));
    testData.isRunCamSplitOpenPortSupported = true;
    testData.isRunCamSplitPortConfigurated = true;
    ...
    testData.maxTimesOfRespDataAvailable = 0;
    uint8_t responseData[] = { 0xCC, 0x01, 0x37, 0x90 };
    uint8_t respDataOfGettingColumnsDetail[] = { 0xCC, 0x00, 0x00, 0x1E, 0x1E, 0x1E, 0x01, 0xdf };
    uint8_t respDataOfGettingTVModeDetail[] = { 0xCC, 0x00, 0x09, 0x00, 0x4E, 0x54, 0x53, 0x43, 0x3B, 0x50, 0x41, 0x4C, 0x00, 0xf9 };
    ...
    addResponseData(respDataOfGettingColumnsDetail, sizeof(respDataOfGettingColumnsDetail), true); // get column counts response
    addResponseData(respDataOfGettingTVModeDetail, sizeof(respDataOfGettingTVModeDetail), true); // get tv mode response
    addResponseData(respDataOfWriteBFCharset, sizeof(respDataOfWriteBFCharset), true); // update charset with BF
...
TEST(RCSplitTest, TestOSDDeviceSupportWithGetColumnsFailed)
{
    memset(&testData, 0, sizeof(testData));
    testData.isRunCamSplitOpenPortSupported = true;
    testData.isRunCamSplitPortConfigurated = true;
    ...
    addResponseData(responseData, sizeof(responseData), true); // device init response
    addResponseData(respDataOfGettingColumnsDetail, sizeof(respDataOfGettingColumnsDetail), true); // get column counts response
    addResponseData(respDataOfWriteDataSuccess, sizeof(respDataOfWriteDataSuccess), true); // update TV mode with Pal
    addResponseData(respDataOfWriteDataFailed, sizeof(respDataOfWriteDataFailed), true); // update charset with BF
    vcdProfile_t profile;
    memset(&profile, 0, sizeof(vcdProfile_t));
...
TEST(RCSplitTest, TestOSDDeviceSupportWithPal)
{
    memset(&testData, 0, sizeof(testData));
    testData.isRunCamSplitOpenPortSupported = true;
    testData.isRunCamSplitPortConfigurated = true;
    ...
    addResponseData(respDataOfGettingColumnsDetail, sizeof(respDataOfGettingColumnsDetail), true); // get column counts response
    addResponseData(respDataOfWriteDataSuccess, sizeof(respDataOfWriteDataSuccess), true); // update TV mode with Pal
    addResponseData(respDataOfWriteDataSuccess, sizeof(respDataOfWriteDataSuccess), true); // update charset with BF
    vcdProfile_t profile;
    memset(&profile, 0, sizeof(vcdProfile_t));
    profile.video_system = VIDEO_SYSTEM_PAL; // use auto detect
    bool r = rcdeviceOSDInit(&profile);
...
TEST(RCSplitTest, TestOSDDeviceSupportWithWriteTVModeFailed)
{
    memset(&testData, 0, sizeof(testData));
    testData.isRunCamSplitOpenPortSupported = true;
    testData.isRunCamSplitPortConfigurated = true;
    ...
    addResponseData(responseData, sizeof(responseData), true); // device init response
    addResponseData(respDataOfGettingColumnsDetail, sizeof(respDataOfGettingColumnsDetail), true); // get column counts response
    addResponseData(respDataOfWriteDataFailed, sizeof(respDataOfWriteDataFailed), true); // update TV mode with Pal
    addResponseData(respDataOfWriteDataSuccess, sizeof(respDataOfWriteDataSuccess), true); // update charset with BF
    vcdProfile_t profile;
    memset(&profile, 0, sizeof(vcdProfile_t));
    profile.video_system = VIDEO_SYSTEM_PAL; // use auto detect
...
TEST(RCSplitTest, TestOSDDeviceSupportWithWriteCharsetFailed)
{
    memset(&testData, 0, sizeof(testData));
    testData.isRunCamSplitOpenPortSupported = true;
    testData.isRunCamSplitPortConfigurated = true;
...
TEST(RCSplitTest, TestDeviceSettingAccessProtocol)
{
